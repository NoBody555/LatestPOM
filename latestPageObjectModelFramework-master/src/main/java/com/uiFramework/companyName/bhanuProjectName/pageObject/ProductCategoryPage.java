...
public class ProductCategoryPage {
	...
	@FindBy(xpath="//ul[@class='color_to_pick_list clearfix']/li/a")
	...
	public void selectColor(String data){
		...
		log.info("Clicked on the item "+data);
		try {
			Thread.sleep(7000);
	...
	public void verifyColor(String bgColor,String elementName) {
		
		boolean changeColor = false;
		int counter = 0;
		
		for(WebElement all: allColors) {
			
		   String allCssValues =    all.getCssValue("background");
		   
		        String colors[]  =     allCssValues.split("n");
		         String hexValue =  Color.fromString(colors[0]).asHex();
		         
		         
		         if(hexValue.toUpperCase().equals(bgColor)) {
		        	 
		        	  counter++;
		        	  changeColor = true;
		        	  System.out.println("Background: "+hexValue);
		         }else
		        	 changeColor = false;
		  ...
		  String text =   driver.findElement(By.xpath("//a[text()='"+elementName+"']")).getText();
		  System.out.println(text);
		
		   if(changeColor) 
			   
			   System.out.println("the no of items present with this color are: "+counter);
